...
if (isset($_POST['add_to_cart'])) {
    ...
    $cart_check = mysqli_query($con, "SELECT quantity FROM cart WHERE userid = $user_id AND itemid = $item_id");
    $current_in_cart = 0;
    if (mysqli_num_rows($cart_check) > 0) {
        $cart_data = mysqli_fetch_array($cart_check);
        $current_in_cart = $cart_data['quantity'];
    }

    // Check if total would exceed stock
    $total_quantity = $current_in_cart + $quantity_to_add;
    if ($total_quantity > $item['quantity']) {
        $available = $item['quantity'] - $current_in_cart;
        echo "<script>alert('Can only add $available more of this item!'); window.location = 'index.php?pg=item_details&id=$item_id';</script>";
        exit();
    }

    // Add to cart
    if ($current_in_cart > 0) {
        mysqli_query($con, "UPDATE cart SET quantity = $total_quantity WHERE userid = $user_id AND itemid = $item_id");
    } else {
        mysqli_query($con, "INSERT INTO cart (userid, itemid, quantity) VALUES ($user_id, $item_id, $quantity_to_add)");
    }
    ...
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a
                    href="index.php?category=<?php echo $item['categoryid']; ?>"><?php echo strtoupper($item['category_name']); ?></a>
            </li>
                ...
                <div class="card-body p-4">
                    ...
                    if (isset($_SESSION['email'])) {
                        ...
                        if ($user_query && mysqli_num_rows($user_query) > 0) {
                            $user_data = mysqli_fetch_array($user_query);
                            $cart_check = mysqli_query($con, "SELECT quantity FROM cart WHERE userid = {$user_data['userid']} AND itemid = $item_id");
                            if (mysqli_num_rows($cart_check) > 0) {
                                $cart_data = mysqli_fetch_array($cart_check);
                                $available_quantity = $item['quantity'] - $cart_data['quantity'];
                            }
                        }
                    }
                    ?>
                    ...
                    <div class="col-sm-6 col-md-3">
                        <div class="card h-100 shadow-sm">
                            <?php if (!empty($related['image'])) { ?>
                                <img src="<?php echo $related['image']; ?>" class="card-img-top"
                                    style="height: 200px; object-fit: cover;"
                                    alt="<?php echo htmlspecialchars($related['itemname']); ?>">
                            <?php } else { ?>
                            ...
                            <div class="card-body d-flex flex-column">
                                ...
                                <div class="mt-auto">
                                    <a href="index.php?pg=item_details&id=<?php echo $related['itemid']; ?>"
